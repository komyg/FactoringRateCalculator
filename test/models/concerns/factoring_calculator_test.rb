...
class FactoringCalculatorTest < ActiveSupport::TestCase
  ...
  test "should calculate a receivable value using the default rate" do
    value = DummyClass.value_default_rate(1500, 28)
    assert_equal 1425.75, value

    value = DummyClass.value_default_rate(1500, 10)
    assert_equal 1475.25, value
  end

  test "should raise an error if the title term is less or equal to 0" do
    term = 0
    assert_raises("The tilte term of: #{term} must be greater than 0") { DummyClass.value_default_rate(1500, term) }
  ...
  test "should raise an error if the tile value is less or equal to 0" do
    title_value = 0
    assert_raises("The title value of: #{title_value} must be greater than 0") { DummyClass.value_default_rate(title_value, 10) }
